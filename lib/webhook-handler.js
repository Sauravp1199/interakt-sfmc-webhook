...
function verifySignature(payload, signature, secret) {
    ...
    if (!secret) {
        logger.warn('INTERAKT_SECRET not configured - skipping signature verification', {
            warning: 'This should be set in production',
            action: 'Set INTERAKT_SECRET or LOCAL_INTERAKT_SECRET in .env file'
        });
        return true;
    }
    ...
    if (!isValid) {
        logger.error('SIGNATURE VALIDATION FAILED: Invalid signature', {
            ...
            action: 'Verify that INTERAKT_SECRET matches the secret configured in Interakt webhook settings',
            hint: 'The signature is computed as HMAC-SHA256(JSON.stringify(payload), secret)'
        });
    }
...
function validatePayload(payload) {
    ...
    if (!payload.type || typeof payload.type !== 'string') {
        errors.push('Missing or invalid "type" field (string required)');
    }

    if (!payload.data || typeof payload.data !== 'object') {
        errors.push('Missing or invalid "data" field (object required)');
    }

    // Validate data.customer.traits.whatsapp_opted_in is boolean if present
    if (payload.data?.customer?.traits?.whatsapp_opted_in !== undefined) {
        const optedIn = payload.data.customer.traits.whatsapp_opted_in;
        if (typeof optedIn !== 'boolean' && optedIn !== 'true' && optedIn !== 'false') {
            errors.push('data.customer.traits.whatsapp_opted_in must be a boolean value');
        }
    }

    // Validate timestamp format if present
    if (payload.timestamp && typeof payload.timestamp === 'string') {
        const isValidDate = !isNaN(Date.parse(payload.timestamp));
        if (!isValidDate) {
            errors.push('Invalid timestamp format (ISO 8601 expected)');
        }
    }
...
async function handleWebhook(payload, headers) {
    ...
    if (!validation.valid) {
        logger.error('WEBHOOK ERROR: Invalid payload structure', {
            errors: validation.errors,
            action: 'Fix the payload structure and retry'
        });
        const error = new Error('Invalid payload: ' + validation.errors.join(', '));
    ...
    if (await isProcessed(fullUniqueId)) {
