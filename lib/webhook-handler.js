...
// NOTE: Idempotency cache functions removed - all webhooks are processed without cache checks
...
async function handleWorkflowResponse(payload, dataExtensionKey) {
    ...
    logger.info('Workflow response processed', {
        totalSteps: qaArray.length,
...
async function handleWebhook(payload, headers, options = {}) {
    ...
    logger.info('Webhook received', { type, timestamp, hasSignature: !!headers['x-interakt-signature'] });
    ...
    // 2. Get Data Extension key from environment
    // NOTE: Idempotency checks DISABLED - all records will be inserted regardless of duplicates
    ...
    try {
        if (type === WEBHOOK_TYPES.WORKFLOW_RESPONSE_UPDATE) {
            ...
            logger.info('Processing webhook', {
                eventId: deData.event_id,
                type: deData.type,
                fieldsCount: nonEmptyFields.length
            });

            await processToSFMC(dataExtensionKey, deData);
        }

        // 6. Webhook processing completed (no cache marking - insert-only mode)
        logger.info('Webhook processed', { type });

    } catch (error) {
        logger.error('Webhook processing failed: ' + error.message, { type });
        throw error;
    }
}
