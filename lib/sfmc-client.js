...
async function getAccessToken() {
    ...
    // Request token
    logger.info('Requesting SFMC token');
    ...
    try {
        ...
        logger.error('SFMC token request failed: ' + error.message);
...
/**
 * Build REST API payload for Data Extension INSERT (with required keys object)
 * Format: [{ keys: {...}, values: {...} }]
 * NOTE: SFMC API requires keys object. No cache checking on application side.
 * Primary Key: event_id (as configured in SFMC Data Extension)
 */
function buildRestPayload(data, primaryKeyField = 'event_id') {
    const keys = {};
    const values = {};
...
async function insertToSFMC(dataExtensionKey, data) {
    ...
    // Log single record insert
    logger.info('Inserting single record to SFMC', {
        dataExtensionKey,
        recordId: data.event_id,
        fieldCount: Object.keys(payload[0].values).length
    });

    try {
        // Log detailed request body to debug file
        logger.debugRequest({
            type: 'SFMC SINGLE INSERT REQUEST',
            url: restUrl,
            method: 'POST',
            requestHeaders: {
                'Content-Type': 'application/json',
        ...
        // Log detailed response body to debug file
        logger.debugRequest({
            type: 'SFMC SINGLE INSERT RESPONSE (SUCCESS)',
        ...
        if (response.status === 200 || response.status === 201 || response.status === 202) {
            logger.info('Record inserted successfully', { recordId: data.event_id, status: response.status });
        ...
        // Log detailed error response to debug file
        logger.debugRequest({
            type: 'SFMC SINGLE INSERT RESPONSE (ERROR)',
            url: restUrl,
            method: 'POST',
            statusCode: error.response ? error.response.status : 'NO_RESPONSE',
        ...
        logger.error('SFMC insert failed: ' + error.message, { status: error.response?.status });
        throw error;
    }
}
 ...
 * Primary Key: event_id (as configured in SFMC Data Extension)
 */
async function batchUpsertToSFMC(dataExtensionKey, dataArray, primaryKeyField = 'event_id', retryCount = 0) {
...
async function batchUpsertWithRetry(
    ...
    // ===== STEP 1: BUILD PAYLOAD (with required keys object for SFMC API) =====
    // NOTE: SFMC API requires keys object. No cache checking on application side (already removed).
    ...
    logger.info('Sending batch to SFMC', {
        recordCount: payload.length,
        payloadSize: payloadValidator.formatBytes(payloadSize),
    ...
    try {
        ...
        // Log detailed request body to debug file
        logger.debugRequest({
            type: 'SFMC BATCH INSERT REQUEST (NO UPSERT)',
            url: restUrl,
            method: 'POST',
            requestHeaders: {
                'Content-Type': 'application/json',
        ...
        // Log detailed response body to debug file
        logger.debugRequest({
            type: 'SFMC BATCH INSERT RESPONSE (SUCCESS)',
            statusCode: response.status,
            responseHeaders: response.headers,
            responseBody: response.data
        });
        ...
        // Log detailed error response to debug file
        logger.debugRequest({
            type: 'SFMC BATCH INSERT RESPONSE (ERROR)',
            url: restUrl,
            method: 'POST',
            statusCode: error.response ? error.response.status : 'NO_RESPONSE',
            responseHeaders: error.response ? error.response.headers : null,
        ...
        logger.error('SFMC batch insert failed: ' + classification.message, {
